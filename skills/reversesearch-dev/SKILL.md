...
```
created → dev-designing → dev-reviewing → dev-confirmed → dev-coding → dev-done → testing → test-reviewing → test-done
                                                          ↓
                                                    测试失败 → dev-fixing → dev-done（循环）
```
...
**⚠️ 完整状态列表（12个）**：`created`, `dev-designing`, `dev-reviewing`, `dev-confirmed`, `dev-coding`, `dev-done`, `dev-fixing`, `dev-waiting`, `testing`, `test-reviewing`, `test-done`, `done`

### 更新方式

**⚠️ 脚本路径**：`/Users/apple/.hermes/profiles/manager/skills/git-task-coordination/templates/task_manager.py`

```bash
TM="python3 /Users/apple/.hermes/profiles/manager/skills/git-task-coordination/templates/task_manager.py"

# 更新状态
$TM transition TASK-ID --to STATUS

# 或手动更新
cd /Users/apple/work/tc-flight-int-reversepromotion
TASK_FILE="hermes-tasks/REQ-XXX/tasks/TASK-XXX.md"
sed -i '' 's/^status: 旧状态$/status: 新状态/' "$TASK_FILE"
git add "$TASK_FILE" && git commit -m "chore(TASK-XXX): 状态变更 旧→新" && git push
```
...
```bash
# 方式1：使用 task_manager.py
$TM list --project tc-flight-int-reversesearch

# 方式2：使用 find
find /Users/apple/work/tc-flight-int-reversepromotion/hermes-tasks -name "TASK-*.md" -type f -exec grep -l "^project: tc-flight-int-reversesearch$" {} \;
```
...
### P58. lark-cli 不可用时的 Feishu API 调用（2026-06-12 发现）
**严重程度：🟡 中**

`lark-cli` 可能未安装或不在 PATH 中。需要直接调用 Feishu API 时，必须用 `curl -k`（跳过 SSL 证书验证），因为内网环境有自签名证书。
