#!/usr/bin/env bash
# Pull latest distribution (hermes profile update).
# On divergence: try auto-merge + publish; on merge conflict, agent-conflict-branch.sh.
#
# Usage: skills/hermes-distribution/scripts/update.sh
set -euo pipefail
...
if [[ "$LOCAL_NEEDS_PUBLISH" == true ]] && [[ "${REMOTE_AHEAD:-0}" -gt 0 ]]; then
  ...
  if [[ "${CONFLICT_STRATEGY}" == "distribution_branch" ]]; then
    ...
    chmod +x "$SKILL_SCRIPTS/agent-divergence-merge.sh"
    chmod +x "$SKILL_SCRIPTS/agent-conflict-branch.sh"
    set +e
    MERGE_OUTPUT="$("$SKILL_SCRIPTS/agent-divergence-merge.sh" 2>&1)"
    MERGE_EXIT=$?
    set -e
    ...
    if [[ "$MERGE_EXIT" -eq 2 ]]; then
      exec "$SKILL_SCRIPTS/agent-conflict-branch.sh"
    fi
    printf '%s\n' "$MERGE_OUTPUT" >&2
    exit 1
  fi
...
if [[ "$LOCAL_NEEDS_PUBLISH" == true ]]; then
  echo "Error: 本机有未发布改动，请先执行 $PUBLISH_CMD" >&2
